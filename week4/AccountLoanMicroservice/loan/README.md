# Loan Microservice - Cognizant DN 5.0 Deep Skilling

## Project Overview
The **Loan Microservice** is a lightweight, independent Spring Boot application developed as part of the Cognizant Digital Nurture (DN) 5.0 Deep Skilling program. It focuses on handling user loan details and exposing them via a RESTful API.
...
## Project Structure
Below is the directory structure of the Loan Microservice project:
...
│   │   │       └── cognizant/
│   │   │           └── loan/
│   │   │               ├── LoanApplication.java (Main boot class)
│   │   │               ├── controller/
│   │   │               │   └── LoanController.java (REST Controller)
│   │   │               └── model/
│   │   │                   └── Loan.java (Data Model POJO)
│   │   └── resources/
...
### Files Explained:
*   [pom.xml](file:///c:/luffy/LPUU/Projects/Cognizant/week4/AccountLoanMicroservice/loan/pom.xml): Declares all dependencies needed for Spring Boot, Web, and DevTools.
*   [application.properties](file:///c:/luffy/LPUU/Projects/Cognizant/week4/AccountLoanMicroservice/loan/src/main/resources/application.properties): Configures port configuration (`server.port=8081`) and application metadata.
*   [LoanApplication.java](file:///c:/luffy/LPUU/Projects/Cognizant/week4/AccountLoanMicroservice/loan/src/main/java/com/cognizant/loan/LoanApplication.java): Main entry point to bootstrap and launch the Spring Boot application.
*   [Loan.java](file:///c:/luffy/LPUU/Projects/Cognizant/week4/AccountLoanMicroservice/loan/src/main/java/com/cognizant/loan/model/Loan.java): Data carrier representing loans.
*   [LoanController.java](file:///c:/luffy/LPUU/Projects/Cognizant/week4/AccountLoanMicroservice/loan/src/main/java/com/cognizant/loan/controller/LoanController.java): REST Controller that handles incoming HTTP requests and serves loan details.

## API
*   **Method**: `GET`
*   **Endpoint**: `/loans/{number}`
*   **Request URL Example**: `http://localhost:8081/loans/H00987987972342`
*   **Response Payload**:
    ```json
    {
...
*   Select **Open** and import the `loan` project folder as a **Maven Project**.

### Step 3: Clean and Install
Open a terminal in the root directory of the loan project and run:
```bash
mvn clean install
```

### Step 4: Run Application
*   Option A: Find [LoanApplication.java](file:///c:/luffy/LPUU/Projects/Cognizant/week4/AccountLoanMicroservice/loan/src/main/java/com/cognizant/loan/LoanApplication.java) in your IDE, right-click, and select **Run**.
...
*   **Browser**: Open `http://localhost:8081/loans/H00987987972342`
*   **Postman**: Send a `GET` request to `http://localhost:8081/loans/H00987987972342`
...
### 1. Model Class: [Loan.java](file:///c:/luffy/LPUU/Projects/Cognizant/week4/AccountLoanMicroservice/loan/src/main/java/com/cognizant/loan/model/Loan.java)
It contains fields representing the loan details: `number` (String), `type` (String), `loan` (double), `emi` (double), and `tenure` (int). It includes a default constructor, parameterized constructor, and getter/setter methods.

### 2. REST Controller: [LoanController.java](file:///c:/luffy/LPUU/Projects/Cognizant/week4/AccountLoanMicroservice/loan/src/main/java/com/cognizant/loan/controller/LoanController.java)
*   **`@RestController`**: Combines `@Controller` and `@ResponseBody`. Tells Spring Boot that this class is a controller and the return values should be serialized directly to the JSON response body.
*   **`@GetMapping("/loans/{number}")`**: Maps HTTP GET requests on `/loans/{number}` to this method.
*   **`@PathVariable String number`**: Extracts the value of `{number}` from the URL path and binds it to the method parameter `number`.
*   **`getLoan(String number)`**: Instantiates a new `Loan` object passing the number from the path and dummy static values (`"Car"`, `400000.0`, `3258.0`, `18`), returning it directly.

### 3. Application Main: [LoanApplication.java](file:///c:/luffy/LPUU/Projects/Cognizant/week4/AccountLoanMicroservice/loan/src/main/java/com/cognizant/loan/LoanApplication.java)
*   **`@SpringBootApplication`**: A convenience annotation enabling component scan and configuration scanning.
*   **`SpringApplication.run(LoanApplication.class, args)`**: Bootstraps the application and runs on the port specified in properties (`8081`).

## Why server.port=8081?
...
*   Since the Account service runs on port `8080`, we configure the Loan service to run on port `8081` in [application.properties](file:///c:/luffy/LPUU/Projects/Cognizant/week4/AccountLoanMicroservice/loan/src/main/resources/application.properties) to allow both microservices to run side-by-side without network conflicts.

## Common Errors

1.  **Port Already in Use (`BindException`)**:
    *   *Cause*: Another application (e.g. Account service or another instance) is already running on port `8081`.
    *   *Fix*: Stop the other application, or edit `server.port` in `application.properties` to a different free port.
2.  **404 Not Found**:
    *   *Cause*: Entering the wrong URL path, e.g., `/loan` instead of `/loans`.
...
**Answer**: By defining the property `server.port=8081` in the [application.properties](file:///c:/luffy/LPUU/Projects/Cognizant/week4/AccountLoanMicroservice/loan/src/main/resources/application.properties) or `application.yml` file.
