...
This project contains two Spring Boot microservices designed to showcase REST APIs, database persistence in MySQL, and synchronous inter-service communication using `WebClient` (Spring WebFlux).
...
4. **Spring WebFlux (WebClient)**: For reactive/non-blocking HTTP communication (used synchronously in this blocking servlet-based context).
5. **MySQL Database**: Default persistence storage (with automated schema creation and DB provisioning using `createDatabaseIfNotExist=true`).
6. **H2 Database**: Provided as an option in `application.properties` for easy in-memory zero-configuration testing.
...
Navigate to the root directory `c:\luffy\LPUU\Projects\Cognizant` (or use maven root command) and compile the project to ensure everything is downloaded and built correctly:
```bash
mvn clean install -DskipTests
```

### Step 2: Start the User Service
Go to the `week4/Microservices/user-service` directory and run:
```bash
mvn spring-boot:run
```
...
Go to the `week4/Microservices/order-service` directory and run:
```bash
mvn spring-boot:run
```
Alternatively, run the jar file:
